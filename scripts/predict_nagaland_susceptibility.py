"""
Nagaland Full-Area Susceptibility Prediction
Loads trained XGBoost model, runs inference on every pixel, saves GeoTIFF + PNG.
Output: data/processed/results/nagaland/nagaland_susceptibility.tif/.png
"""
...
MODEL_PATH  = "models/nagaland/nagaland_xgboost.pkl"
...
print("=" * 60)
print("Nagaland Full-Area Susceptibility Prediction")
print("=" * 60)
...
# ---- Save GeoTIFF -------------------------------------------------------
out_tif = os.path.join(OUT_DIR, "nagaland_susceptibility.tif")
meta = ref_meta.copy()
...
# Plot event locations (convert UTM to same space)
events_gdf = gpd.read_file("data/processed/labels/nagaland/nagaland_event_buffers.gpkg")
event_pts = events_gdf.geometry.centroid
ax.scatter(event_pts.x, event_pts.y, c='white', edgecolors='black', s=25, zorder=5,
           linewidths=0.8, alpha=0.9, label=f'Landslide events (n=101)')

ax.set_title("Nagaland Landslide Susceptibility Map\n"
...
# Add NH-29 annotation
mid_x = (extent[0] + extent[1]) / 2
mid_y = (extent[2] + extent[3]) / 2
ax.annotate("NH-29 / NH-2\ncorridor", xy=(mid_x, mid_y - 50000),
            fontsize=9, color='navy', style='italic', ha='center',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='lightyellow', alpha=0.8))

out_png = os.path.join(OUT_DIR, "nagaland_susceptibility_map.png")
plt.tight_layout()
plt.savefig(out_png, dpi=150, bbox_inches='tight')
plt.close()
print(f"Map saved: {out_png}")

print("\nDONE - Susceptibility prediction complete!")
