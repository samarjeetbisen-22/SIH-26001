"""
Assam XGBoost Susceptibility Model
Train on 6-factor balanced dataset (105 landslide events, 150m buffer).
Outputs: trained model, metrics JSON, feature importance + performance plots.
"""
...
# ---- CONFIG --------------------------------------------------------------
TRAIN_CSV   = "data/processed/training/assam/assam_training.csv"
MODEL_DIR   = "models/assam"
RESULTS_DIR = "data/processed/results/assam"
RANDOM_SEED = 42
...
print("=" * 60)
print("Assam XGBoost Susceptibility Model")
print("=" * 60)
...
print(f"  Positive (landslide): {int(df['target'].sum()):,}")
print(f"  Negative (background): {int((df['target'] == 0).sum()):,}")
...
# ---- Save model + metrics -----------------------------------------------
model_path = os.path.join(MODEL_DIR, "assam_xgboost.pkl")
...
metrics = {
    ...
    "label_strategy": "150m buffer around 105 point events",
    "utm_crs": "EPSG:32646",
}
metrics_path = os.path.join(RESULTS_DIR, "assam_model_report.json")
...
axes[0].set_title("Feature Importance -- Assam XGBoost")
axes[0].invert_yaxis()
...
axes[1].set_title("Model Performance -- Assam XGBoost")
axes[1].legend()
...
plt.tight_layout()
out_png = os.path.join(RESULTS_DIR, "assam_model_performance.png")
plt.savefig(out_png, dpi=150, bbox_inches='tight')
plt.close()
print(f"Plot saved: {out_png}")

print("\nDONE - Assam model trained successfully!")
